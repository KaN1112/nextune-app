...
export function render() {
  return (
    ...
    `<div class="section-space">${panel("システムの稼働状況", '<canvas class="chart" id="activity-chart" role="img" aria-label="過去60秒のCPU・メモリ使用率"></canvas><div class="chart-axis"><span>60秒前</span><span>0–100% · 現在</span></div>', '<div class="chart-legend"><span>CPU</span><span class="ram">RAM</span></div>')}</div><div class="grid two section-space">${panel(
      "温度",
      definition([
        ["CPU温度", "取得できません"],
        ["GPU温度", "取得できません"],
      ]) +
        '<p class="muted section-space">追加ドライバーやハードウェア操作ツールは導入しません。</p>',
    )}${panel("システム詳細", '<div id="system-details"></div>')}</div>`
  );
}
